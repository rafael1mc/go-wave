...
func (g *Game) calculateReflectedWaves(x, y float64, source WaveSource) float64 {
	...
	// Calculate the distance from source to center
	// sourceDx := source.x - float64(centerX)
	// sourceDy := source.y - float64(centerY)
	// sourceDistFromCenter := math.Sqrt(sourceDx*sourceDx + sourceDy*sourceDy)

	// The outgoing wave reaches the boundary at all angles around the source
	// We need to consider when the wave from source reaches each point on the boundary

	totalReflectedHeight := 0.0

	// Sample multiple angles around the boundary to get reflections
	for angleStep := 0; angleStep < 360; angleStep += 5 {
		angle := float64(angleStep) * math.Pi / 180.0
