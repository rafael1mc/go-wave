...
func (g *Game) calculateWaveHeight(x, y float64) float64 {
	...
	for _, source := range g.waveSources {
		...
		// Calculate distance from source to center
		...
		// Distance the wave needs to travel to reach boundary from source
		maxDistToBoundary := float64(shapeRadius) - sourceDistFromCenter

		// Check if wave has reached the boundary
		if waveFront > maxDistToBoundary {
