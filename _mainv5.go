...
func (g *Game) Draw(screen *ebiten.Image) {
	...
	// Draw grid of points
	for px := centerX - shapeRadius; px < centerX+shapeRadius; px += gridSize {
		for py := centerY - shapeRadius; py < centerY+shapeRadius; py += gridSize {
			dx := px - centerX
			dy := py - centerY
			...
			if distFromCenter < shapeRadius {
				height := g.calculateWaveHeight(float64(px), float64(py))

				// Map height to color
				var r, gb, b uint8
				if height > 0 {
					// Positive: light blue
					intensity := int(height * 200)
					r = 100
					gb = uint8(150 + intensity/2)
					b = 220
				} else {
